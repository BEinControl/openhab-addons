...
import static org.openhab.binding.dmx.internal.DmxBindingConstants.CHANNEL_CONTROL;
import static org.openhab.binding.dmx.internal.DmxBindingConstants.CHANNEL_SWITCH;
import static org.openhab.binding.dmx.internal.DmxBindingConstants.THING_TYPE_CHASER;
...
public class ChaserThingHandler extends DmxThingHandler {
    ...
    public void initialize() {
        ...
        // Code that I want to modify
        if (!configuration.steps.isEmpty()) {
            values = ValueSet.parseChaseConfig(configuration.steps);

            // byte[][] array = GenerateDmxByteArray(values, bridgeHandler.getRefreshTime());
            // logger.trace("Byte Array: ".concat(array.toString()));
            if (!values.isEmpty()) {
    ...
    // public byte[][] GenerateDmxByteArray(List<ValueSet> values, int refreshTime) {
    // byte[][] generated;
    // int animationLength = 0;

    // for (ValueSet valueSet : values) {
    // animationLength += valueSet.getFadeTime() + valueSet.getHoldTime();
    // }

    // // I assume that something with no fade or hold time does not play
    // if (animationLength <= 0) {
    // logger.warn("Chaser has no steps");
    // return new byte[0][];
    // }
    ...
    // int frameIndex = 0;
    // int nextStepIndex = 1;
    // ValueSet previousFrame = null;
    // ValueSet currentFrame = values.get(0);

    // // Creates a new byte array with however many frames there are
    // generated = new byte[(int) (animationLength / refreshTime)][];

    // for (int animationTime = 0; animationTime <= animationLength; animationTime += refreshTime) {
    // if (animationTime > startTime + currentFrame.getFadeTime() + currentFrame.getHoldTime()) {
    // // Gets the next frame and saves the previous frame
    // previousFrame = values.get(nextStepIndex - 1);
    // currentFrame = values.get(nextStepIndex);
    // nextStepIndex++;

    // startTime += previousFrame.getFadeTime() + previousFrame.getHoldTime();
    // }
    // // This should be the size of the channels
    // byte[] append = new byte[currentFrame.size()];

    // for (int index = 0; index < append.length; index++) {
    // // Just sets the value
    // if (currentFrame.getFadeTime() == 0)
    // append[index] = (byte) currentFrame.getValue(index);
    // // Calculate fade value then sets the value to it
    // else {
    // int startValue = 0;
    // if (previousFrame != null)
    // startValue = previousFrame.getValue(index);

    // append[index] = (byte) (startValue + ((currentFrame.getValue(index) - startValue)
    // / currentFrame.getFadeTime() * Math.min((double) (animationTime - startTime), 1.0)));
    // }
    // }

    // // Adds the frame to the generated byte array
    // generated[frameIndex] = append;
    // frameIndex++;
    // }
